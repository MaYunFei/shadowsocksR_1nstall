...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://github.com/shadowsocksrr/shadowsocksr/archive/3.2.2.tar.gz"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/ToyoDAdoubi/shadowsocksr.git
	#[[ ! -e ${ssr_folder} ]] && echo -e "${Error} ShadowsocksR服务端 下载失败 !" && exit 1
	[[ ! -e "3.2.2.tar.gz" ]] && echo -e "${Error} ShadowsocksR服务端 压缩包 下载失败 !" && rm -rf 3.2.2.tar.gz && exit 1
	tar -zxvf "3.2.2.tar.gz"
	[[ ! -e "/usr/local/shadowsocksr-3.2.2/" ]] && echo -e "${Error} ShadowsocksR服务端 解压失败 !" && rm -rf 3.2.2.tar.gz && exit 1
